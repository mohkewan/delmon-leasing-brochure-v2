...
// SVG أيقونات خطية موحدة بلون الهوية
...
const SvgLock = ({ color, size = 14 }: { color: string; size?: number }) => (
...
const SvgParking = ({ color, size = 14 }: { color: string; size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ display: "inline-block", verticalAlign: "middle", flexShrink: 0 }}>
    <rect x="3" y="3" width="18" height="18" rx="2"/>
    <path d="M9 17V7h4a3 3 0 0 1 0 6H9"/>
  </svg>
);
const SvgStar = ({ color, size = 14 }: { color: string; size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ display: "inline-block", verticalAlign: "middle", flexShrink: 0 }}>
        ...
        <div style={{ width: 490, background: BG_PANEL, clipPath: "polygon(0 0, 100% 0, 96% 100%, 0 100%)", display: "flex", flexDirection: "column", position: "relative", zIndex: 2, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, padding: "14px 22px 0", display: "flex", flexDirection: "column", gap: 7, overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[
                { v: data.totalArea ? `${parseFloat(data.totalArea).toLocaleString("en-US")}` : "—", l: "المساحة م²" },
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, marginBottom: 16 }}>
              ...
              {data.units.map((unit, idx) => {
                ...
                return (
                  <tr key={unit.id} style={{ background: idx % 2 === 0 ? BG_PANEL : BG_SECTION, borderBottom: `1px solid ${GOLD_BORDER}` }}>
                    ...
                    <td style={{ padding: "8px 6px", textAlign: "center", fontWeight: 800, color: TEXT_DARK }}>{unit.area ? parseFloat(unit.area).toLocaleString("en-US") : "—"}</td>
                    ...
                    <td style={{ padding: "8px 6px", textAlign: "center", color: TEXT_MID }}>{unit.pricePerMeter ? parseFloat(unit.pricePerMeter).toLocaleString("en-US") : "—"}</td>
            ...
            <tfoot>
              <tr style={{ background: BG_SECTION, borderTop: `2px solid ${GOLD}` }}>
                ...
                <td style={{ padding: "10px 8px", textAlign: "center", fontWeight: 900, color: GOLD, fontSize: 13 }}>{totalArea.toLocaleString("en-US")} م²</td>
                <td colSpan={4} />
              </tr>
            </tfoot>
          </table>
