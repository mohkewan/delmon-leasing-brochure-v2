      ...
      <div style={{ width: W, height: H, background: BG, display: "flex", flexDirection: "column", pageBreakAfter: "always" }}>
        ...
        <div style={{ padding: "16px 40px", flex: 1, overflow: "hidden" }}>
          <div style={{ marginBottom: 14 }}>
            ...
            <p style={{ color: MID, fontSize: 12, margin: 0 }}>إجمالي {data.units.length} وحدة شاغرة بمساحة إجمالية {totalArea.toLocaleString("ar-SA")} م²</p>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, marginBottom: 16, border: `2px solid ${GREEN_BORDER}`, borderRadius: 8, overflow: "hidden" }}>
              ...
              {data.units.map((unit, idx) => {
                ...
                return (
                  <tr key={unit.id} style={{ background: idx % 2 === 0 ? WHITE : GREEN_PALE, borderBottom: `1px solid ${GREEN_BORDER}` }}>
                    ...
                    <td style={{ padding: "8px 6px", textAlign: "center", fontWeight: 800, color: DARK, border: `1px solid ${GREEN_BORDER}` }}>{unit.area ? parseFloat(unit.area).toLocaleString("ar-SA") : "—"}</td>
                    ...
                    <td style={{ padding: "8px 6px", textAlign: "center", color: MID, border: `1px solid ${GREEN_BORDER}` }}>{unit.pricePerMeter ? parseFloat(unit.pricePerMeter).toLocaleString("ar-SA") : "—"}</td>
            ...
            <tfoot>
              <tr style={{ background: GREEN_PALE, borderTop: `2px solid ${GREEN}` }}>
                ...
                <td style={{ padding: "10px 8px", textAlign: "center", fontWeight: 900, color: GREEN, fontSize: 13, border: `1px solid ${GREEN_BORDER}` }}>{totalArea.toLocaleString("ar-SA")} م²</td>
          ...
          <div style={{ display: "flex", gap: 14 }}>
            {[
              ...
              { label: "إجمالي المساحة الشاغرة", value: totalArea.toLocaleString("ar-SA"), sub: "م²", accent: GREEN_LIGHT },
              { label: "متوسط مساحة الوحدة", value: data.units.length ? Math.round(totalArea / data.units.length).toLocaleString("ar-SA") : "—", sub: "م²", accent: GREEN },
            ].map((c, i) => (
